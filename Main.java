...
public class Main {
    ...
    public static void writeObjectsToFile(Tiger tiger, Penguin penguin, Dolphin dolphin) {
        try (FileOutputStream fos1 = new FileOutputStream("tiger.txt");
            ...
            oos1.writeObject(tiger);
        } catch (IOException e) {
            e.printStackTrace();
        }

        try (FileOutputStream fos2 = new FileOutputStream("penguin.txt");
             ObjectOutputStream oos2 = new ObjectOutputStream(fos2)){
            oos2.writeObject(penguin);
        } catch (IOException e) {
            e.printStackTrace();
        }

        try (FileOutputStream fos3 = new FileOutputStream("dolphin.txt");
             ObjectOutputStream oos3 = new ObjectOutputStream(fos3)){
            oos3.writeObject(dolphin);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void readObjectsFromFile(){
        ...
        try (FileInputStream fis1 = new FileInputStream("tiger.txt");
        ObjectInputStream ois1 = new ObjectInputStream(fis1)){
            tiger = (Tiger) ois1.readObject();
            //System.out.println("Tiger object is serialized successfully.");
        }catch (IOException | ClassNotFoundException e){
            e.printStackTrace();
        }
        System.out.println(tiger);

        try (FileInputStream fis2 = new FileInputStream("penguin.txt");
             ObjectInputStream ois2 = new ObjectInputStream(fis2)){
            penguin = (Penguin) ois2.readObject();
            //System.out.println("Penguin object is serialized successfully.");
        }catch (IOException | ClassNotFoundException e){
            e.printStackTrace();
        }
        System.out.println(penguin);

        try (FileInputStream fis3 = new FileInputStream("dolphin.txt");
             ObjectInputStream ois3 = new ObjectInputStream(fis3)){
            dolphin = (Dolphin) ois3.readObject();
